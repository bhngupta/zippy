...
TEST_F(DatabaseTest, DeletionTest) {
  db->set("key3", "value3");
  db->del("key3");
  EXPECT_EQ(db->get("key3"),
            ""); // Should return empty as the key has been deleted
}
...
TEST_F(DatabaseTest, UpdateExpirationTimeTest) {
  ...
  EXPECT_EQ(db->get("key4"),
            "value4"); // Should still be available as TTL was reset
}
...
TEST_F(DatabaseTest, DeleteFromSnapshotTest) {
  ...
  db->del("key1");
  EXPECT_EQ(db->get("key1"),
            ""); // Should return empty as the key has been deleted from both
                 // in-memory and snapshot
