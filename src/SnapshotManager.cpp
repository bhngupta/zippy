...
std::unordered_map<std::string, std::string> SnapshotManager::loadSnapshot() {
  ...
  if (!std::filesystem::exists(snapshotFilePath_)) {
    std::cerr << "Snapshot file does not exist: " << snapshotFilePath_
              << ". Creating a new snapshot file." << std::endl;
...
SnapshotManager::mergeSnapshots(const std::unordered_map<std::string, std::string> &currentSnapshot,
